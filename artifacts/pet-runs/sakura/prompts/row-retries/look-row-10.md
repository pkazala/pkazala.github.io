Create Codex v2 pet look row 10 for `sakura` as exactly 8 full-body frames in this order: 180, 202.5, 225, 247.5, 270, 292.5, 315, 337.5.
...
REPAIR REQUIREMENT AFTER CONTINUITY FAILURE: the previous row's directions were correct, including a clear 180 down and 270 screen-left, but deterministic assembly showed a conspicuous `157.5 -> 180` snap: the 180 pet became much larger, shifted roughly 28 pixels toward center, and changed area by about 30%. Use the attached REGISTERED row-9 reference as the geometry authority. Draw cell 180 at exactly the same visible full-body size, lower-body anchor, baseline, and horizontal registration as row 9 cell 157.5; change only the final small gaze/head increment into unambiguous down. Then keep that exact size and planted anchor through all eight row-10 poses. Preserve the currently passing down-left -> left -> up-left semantic arc, white ceramic identity, blossom ear, bell, and raised paw. Do not re-center 180 independently.
