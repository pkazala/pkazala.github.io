Create Codex pet row `running-right` for `sakura`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Compact full-body white maneki-neko lucky cat; soft ceramic-toy styling; warm friendly face; red collar with a small attached gold bell; one raised beckoning paw; a single readable pink sakura blossom tucked behind one ear; no coin, text, floor, shadow, or detached decoration.. Preserve silhouette, face, palette, material, proportions, markings, and props.
